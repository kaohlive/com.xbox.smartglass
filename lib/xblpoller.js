...
class XblPoller extends EventEmitter {
	...
	async _tickAchievements(authHeader, xuid) {
		...
		if (titles.length === 0) {
			if (this._initial) this._event('Achievements seeding: 0 recent titles returned from history endpoint');
			return;
		}
		...
		const wasInitial = this._initial;
		let checked = 0;
		let newCount = 0;
		...
		if (wasInitial) {
			this._event('Achievements seeded: ' + seenIds.size + ' ids across ' + recentTitles.length + ' recent titles (checked ' + checked + ' unlocks)');
		} else if (newCount === 0 && checked > 0) {
			// Only log "nothing new" the first time it happens after seeding,
			// not every tick — but only at debug-level via this.log, not the
			// event buffer (which should stay event-only).
		}
	...
	async _tickFriends(authHeader) {
		...
		for (const f of friends) {
			...
			if (present && !wasPresent) {
				...
				// Friend stayed online but switched games — surface this for
				// diagnostics. No trigger fires (intentional; we don't have
				// a flow card for it), but the event log helps the user see
				// that game-title extraction *is* working for some entries.
