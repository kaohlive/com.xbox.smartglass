...
// v133 = added a time-based filter on top of the seen-set. Microsoft's
// achievements endpoint is eventually-consistent — between two ticks
// it can return a partially different set of unlocks for the same
// title, which made stale (pre-seed) achievements look 'new' to the
// seen-set check. The seed now also stores the cutoff timestamp, and
// only achievements with timeUnlocked > cutoff can ever fire.
const ACHIEVEMENT_SEED_VERSION = 'v133';
const ACHIEVEMENT_SEED_TIME_KEY = 'xbl_achievements_seed_time';
...
class XblPoller extends EventEmitter {
	...
	async _tickAchievements(authHeader, xuid) {
		...
		// query semantics, wipe it and force a re-seed in this tick. The
		// seed-time cutoff is also cleared so the upcoming seed stamps a
		// fresh threshold.
		...
		if (needsReseed) {
			...
			this.homey.settings.unset(ACHIEVEMENT_SEED_TIME_KEY);
			this._event('Achievements: re-seeding (endpoint params changed in this version, clearing stale seen-set)');
		}

		// Step 1: fetch recently-played titles from titlehub, already sorted
		// by lastTimePlayed descending. The cap bounds the per-title API
		// fan-out in step 2.
		const recentTitles = await xboxapi.getRecentlyPlayedTitles(authHeader, xuid, ACHIEVEMENT_TITLE_CAP);
		if (recentTitles.length === 0) {
			this._emitAchTickSummary(0, 0, 0);
			if (this._initial || needsReseed) {
				this._event('Achievements seeding: 0 recently-played titles returned from titlehub');
				this.homey.settings.set(ACHIEVEMENT_SEED_VERSION_KEY, ACHIEVEMENT_SEED_VERSION);
				this.homey.settings.set(ACHIEVEMENT_SEED_TIME_KEY, Date.now());
			}
			return;
		}
		...
		// Time-based filter. During seeding the cutoff is "now" and every
		// pre-existing unlock falls below it. On subsequent ticks the
		// cutoff is whatever we stored at seed time; only unlocks newer
		// than that can fire, regardless of what the seen-set says. This
		// hardens us against the achievements endpoint being eventually-
		// consistent and returning slightly different sets per tick.
		const seedTime = wasSeeding
			? Date.now()
			: (this.homey.settings.get(ACHIEVEMENT_SEED_TIME_KEY) || 0);
		let checked = 0;
		let newCount = 0;
		...
		for (const t of recentTitles) {
			...
			for (const ach of unlocks) {
				...
				const unlockTs = unlockTime ? Date.parse(unlockTime) : NaN;
				if (Number.isFinite(unlockTs) && (!mostRecent || unlockTs > mostRecent.ts)) {
					mostRecent = {
						ts: unlockTs,
				...
				const alreadySeen = seenIds.has(key);
				if (!alreadySeen) seenIds.add(key);
				if (wasSeeding) continue;
				// Time-cutoff: pre-seed unlocks can NEVER fire, no matter
				// what the seen-set says. This is the real defence against
				// MS returning a slightly different page of unlocks per
				// tick — stale entries are simply ignored.
				if (!Number.isFinite(unlockTs) || unlockTs <= seedTime) continue;
		...
		if (wasSeeding) {
			const mostRecentSuffix = mostRecent
				...
				: '';
			const cutoffIso = new Date(seedTime).toISOString();
			this._event('Achievements seeded: ' + seenIds.size + ' unlocked ids across ' + recentTitles.length + ' titles (checked ' + checked + ') — cutoff: ' + cutoffIso + mostRecentSuffix);
			this.homey.settings.set(ACHIEVEMENT_SEED_VERSION_KEY, ACHIEVEMENT_SEED_VERSION);
			this.homey.settings.set(ACHIEVEMENT_SEED_TIME_KEY, seedTime);
		}
