...
class XboxAuth {
	...
	// Surface the wipe via a Homey notification so the user finds out
	// without having to open the settings panel. The event log already
	// has the gory details — the notification is just a nudge.
	_notifyReauthRequired() {
		try {
			this.homey.notifications.createNotification({
				excerpt: 'Xbox sign-in expired — open the Xbox console companion app settings to sign in again.',
			}).catch((err) => {
				this.log('Could not create reauth notification: ' + err.message);
			});
		} catch (err) {
			this.log('Could not create reauth notification: ' + err.message);
		}
