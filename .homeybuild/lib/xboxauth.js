...
class XboxAuth {
	...
	// Refresh the MSA access token using the stored refresh token.
	async _refreshMsaAccessToken() {
		const refreshToken = this.homey.settings.get(REFRESH_TOKEN_KEY);
		...
		const data = await res.json();
		if (!res.ok) {
			// invalid_grant means the refresh token is dead — force re-auth.
			if (data.error === 'invalid_grant') {
				this.homey.settings.unset(REFRESH_TOKEN_KEY);
			}
			throw new Error(`Refresh failed: ${data.error || res.status} — ${data.error_description || ''}`);
		}
		this._memo.msaAccessToken = data.access_token;
		this._memo.msaAccessExpires = Date.now() + (data.expires_in - 60) * 1000;
		if (data.refresh_token) {
	...
	async getStatus() {
		...
		try {
			...
			return { authenticated: true, gamertag: this.getGamertag() };
		} catch (err) {
